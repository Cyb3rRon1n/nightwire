...
- [ ] **Step 1:** On msi-laptop (or whichever GPU box is current): confirm `image_server/`'s worker actually starts and responds to a direct HTTP call before testing through nightwire at all — isolates "backend integration bug" from "worker itself broken."
- [ ] **Step 2:** Full flow: join → approve character → confirm a real portrait file is written and the sheet/band renders it. Time the swap-and-generate cost, record the real number (spec estimated 6-15s at 512px on this hardware class — confirm or correct).
- [ ] **Step 3:** Send an action that plausibly warrants a scene image, confirm `image_request` fires, a real scene image generates using the approved portrait as reference, renders as a card in the feed.
- [ ] **Step 4:** Confirm the named GPU-swap cost is real and visible: the *next* narrator turn after an image generation should show the cold-load delay (~55-60s per earlier live measurements) — confirm it happens, don't just assume the spec's prediction held.
- [ ] **Step 5:** Eyeball the reference-consistency claim directly — same character, two separate scene generations, does it actually look like the same character? This is the spec's own flagged "honest gap" (no benchmark for this exact hardware/mode) — this step is what actually answers it for real, not a citation.
- [ ] **Step 6:** Check server logs / browser console for errors across the whole pass.
- [ ] **Step 7:** Update `ROADMAP.md`'s Phase 5 entry with what's actually built, actual measured timings, and the real answer to Step 5's consistency check.
- [ ] **Step 8: commit** — `docs: close out Phase 5 verification pass`
