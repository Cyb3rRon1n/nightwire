...
1. **Cyberpunk ruleset design** (designed 2026-08-22, not yet implemented) — the new "SRD": core dice/resolution mechanic, attributes, roles/classes, cyberware/gear system, hacking mechanics, combat resolution, advancement. Full design in `docs/superpowers/specs/2026-08-22-cyberpunk-ruleset-design.md`. Pure design, no code yet. Gates everything below — the engine's data model and the sheet UI both need real mechanics to model before they can be built.
   ...
   - **Four starting roles** (Solo/Netrunner/Fixer/Tech), not a genre-authentic full ten (Cyberpunk RED has ten). **Flagged honestly, not independently researched**: this count/spread was proposed by analogy to oracle's own four-class first slice, not from any research into how many starting archetypes a first playable slice actually needs. Worth revisiting with real research (or just played-and-adjusted) rather than treated as settled.
