...
3. **AI GM / narrator backend** (not started) — local-first (Ollama), tool-calling for mechanical resolution, structured-output-over-native-tool-calling as the starting assumption for local models specifically (external evidence in `docs/superpowers/specs/2026-08-22-core-engine-design.md`'s narrator-backend section — reliability tracks training/quantization, not raw size), a real reliability-measurement harness from the start rather than retrofitted.
