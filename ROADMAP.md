...
4. **Web frontend** (fully designed 2026-08-22, not yet built) — full design in `docs/superpowers/specs/2026-08-22-web-frontend-design.md`. Vite + React 19 + TypeScript + Tailwind. Layout/composer/sheet-overlay UI was mocked up before this project had its own repo (see "Design already settled" below) and carries forward unchanged; the underlying technical architecture was independently researched fresh:
   - **State split by kind, not one library for everything**: server-pushed state (character sheets, NPC list, world/combat state) gets a thin WebSocket-message-to-reducer layer, not Zustand — reaching for a client-state library for state that's actually server-authoritative solves the wrong problem. This corrects the earlier "match open-dungeon's plain useState" note, which only worked for open-dungeon because it has no live multiplayer sync at all.
   - **`react-use-websocket`** (not hand-rolled reconnect logic) in a single root provider, exponential backoff with jitter on reconnect.
   - **Vitest + React Testing Library + MSW's `ws` namespace** — confirmed still the 2026 standard stack, with real first-class WebSocket mocking now built into MSW.
